...
mod tests {
    ...
    fn pick_resumable_sid_falls_back_when_the_frontmatter_transcript_is_gone() {
        ...
        assert_eq!(
            pick_resumable_sid(None, alive, || Some(live.to_string())),
            Some(live.to_string())
        );

        // Nothing resumable anywhere: keep the frontmatter id so the UI still identifies
        // the session (only `resumable` goes false) — but never invent one.
        assert_eq!(
            pick_resumable_sid(Some(dead), alive, || None),
            Some(dead.to_string())
        );
